...
11/22 ガウシアン平滑化を最後にするように、bigloop もいじった．前のverはMiniMini_1122 を参照
		ここに、カーネルをかけて最小化　そして最後にガウシアン！

12/06 カーネル16で最後にガウシアンが良さそうなので、これを
　　　毎回最小二乗ではなく、加工しきってから最小二乗に変更する しかし、これは面倒なので保留　最小二乗のタイミングが面倒すぎ。廃棄
...
char inputfile[] = "Hiroshima_02.jpg"; //入力元ファイル名
char maskfile[] = "Hiroshima_02_mask.bmp"; //入力マスクファイル名

char Infile[] = "Hiroshima_02.jpg"; //最小二乗の理想ファイル名 特に何もなければ入力と同じでよい
...
*/
	...
	cv::Mat beforepre(rows,cols,CV_8UC3);
		...
		for(int i=0 ; i<(rows*cols)  ;i++){
			...
			beforepre.data[i*3]=b1+150.0;
			beforepre.data[i*3+1]=g1+150.0;
			beforepre.data[i*3+2]=r1+150.0;	

			int b = img.data[(i*3 +0)] + b1;
			int g = img.data[(i*3 +1)] + g1;
			int r = img.data[(i*3 +2)] + r1;
...
rgbbefore=beforepre;
		...
		for(int i=0; i<rows*cols ;i++){
		//		cout<<i<<endl;
		rgbbefore.data[i*3] = 150.0+ Ba*(beforepre.data[i*3]-150.0);
				rgbbefore.data[i*3+1] = 150.0+ Ga*(beforepre.data[i*3+1]-150.0);
						rgbbefore.data[i*3+2] = 150.0+ Ra*(beforepre.data[i*3+2]-150.0);
	//	cout<<i<<endl;
		}
...
//			int Inum = depth.data[i];
			...
			if(Inum == 0) rgbgauss = rgbbefore;
			if (Inum == 70) rgbgauss = rgbbefore;
			if (Inum == 140) rgbgauss = rgbgauss6;
			if (Inum == 210) rgbgauss = rgbgauss7;
			if (Inum == 240) rgbgauss = rgbgauss8;

			int b = Mini.data[(i*3 +0)] + rgbgauss.data[(i*3 +0)]-150.0 - (beforepre.data[i*3]-150.0);
			int g = Mini.data[(i*3 +1)] +rgbgauss.data[(i*3 +1)]-150.0 - (beforepre.data[i*3+1]-150.0);
			int r = Mini.data[(i*3 +2)] +rgbgauss.data[(i*3 +2)]-150.0 - (beforepre.data[i*3+2]-150.0);

			adjust(b, g, r);

			afterpre.data[(i*3 +0)] = b;
			afterpre.data[(i*3 +1)] = g;
			afterpre.data[(i*3 +2)] = r;
		}
		...
		//加工後の画像を最小二乗で規格化
//		cv::Mat Mini = MiniMiniRGB3(after,q+1);
		//加工後の画像出力
		char nameafter[40];
...
//	cout<<"a"<<endl;
	...
	//探索範囲の閾値設定
	int  karnel = 8;//探索範囲の腕の長さの閾値
	int  gain = 70; //一マス離れるごとに減らす分 karnel*gain <255 になるように
...
//マーク地点がマスク内かチェック
void checktop(int x, int y,int q){
	cv::Mat mask = cv::imread(maskfile,0);

	if (mask.data[mask.cols*y*16+x*16]>150){

		//cout<<x<<","<<y<<","<<mask.data[mask.cols*y*16+x*16]<<endl;
		char nameafter[40];
		sprintf_s(nameafter,"after_kaisuu%03d.bmp",q);
		//cv::Mat last = cv::imread(nameafter);
		//cv::imshow("kekka",last);
		top = 1;
	}
...
void checktop2(int x, int y,int q){
	cv::Mat mask = cv::imread(maskfile,0);
...
//check top を使って、Saliの高いところを元画像に重ねて出力する関数 改訂 （Sali(白黒) 64 1ch、 元画像（カラー）8 3ch）
cv::Mat Markver2c(cv::Mat Sali,const cv::Mat img,int q){


	//書き込み用画像用意
	int rows = img.rows;
	int cols = img.cols;
	cv::Mat Markimg(rows,cols,CV_8UC3);	
	//元画像をスケール4までダウン処理
	Markimg = DownSample(img , 4);

	cv::imwrite("imgdown.bmp", Markimg);
...
//check top を使って、Saliの高いところを元画像に重ねて出力する関数 改訂 （Sali(白黒) 64 1ch、 元画像（カラー）8 3ch）
cv::Mat Markver2c2(cv::Mat Sali,const cv::Mat img,int q){
	...
	int q1 = q;
	checktop2(maxLoc.x, maxLoc.y,q1);

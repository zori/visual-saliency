...
char inputfile[] = "Hiroshima_11.jpg"; //入力元ファイル名
char maskfile[] = "Hiroshima_11_mask2.bmp"; //入力マスクファイル名

char Infile[] = "Hiroshima_11.jpg"; //最小二乗の理想ファイル名 特に何もなければ入力と同じでよい
...
cv::Mat MiniMiniKK(const cv::Mat edit,int count,const cv::Mat Kyoudo){
	...
	cv::Mat edit2(rows, cols, CV_8UC3);
		...
		//強度係数の絶対値の逆数をかける
		for(i=0 ;i<N; i++){
			int pre = abs(Kyoudo.data[i]-128);
			if(pre==0){
				pre = 1;
			};
			edit2.data[i*3] = ( edit2.data[i*3] / pre );
			edit2.data[i*3+1] = ( edit2.data[i*3+1] / pre );
			edit2.data[i*3+2] = ( edit2.data[i*3+2] / pre );

			img2.data[i*3] = ( img2.data[i*3] / pre );
			img2.data[i*3+1] = ( img2.data[i*3+1] / pre );
			img2.data[i*3+2] = ( img2.data[i*3+2] / pre );
		}
		*/
		...
		for(i=0 ;i<N; i++){
			...
			edit2.data[i*3] = int( edit2.data[i*3] * pre +0.5);
			edit2.data[i*3+1] = ( edit2.data[i*3+1] * pre +0.5 );
			edit2.data[i*3+2] = ( edit2.data[i*3+2] * pre  +0.5);

			img2.data[i*3] = int( img2.data[i*3] * pre +0.5);
			img2.data[i*3+1] = int( img2.data[i*3+1] * pre +0.5);
			img2.data[i*3+2] = int( img2.data[i*3+2] * pre +0.5);
		}

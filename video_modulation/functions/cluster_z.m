...
% May 2015
% 8.5.0.197613 (R2015a)
function T = cluster_z(Z, varargin)
...
%         internal.stats.parseArgs(okargs,dflts,varargin{:});
    % EDIT(zori) varargin{:} is 'cutoff'    [0.5000]    'criterion'    'distance'
    [cutoff maxclust criterion depth setflag] = ...
        parseArgs_z(okargs,dflts,varargin{:});
    % values are: {cutoff maxclust criterion depth setflag}
    %    [0.5000]    [Inf]    'distance'    [2]    [1x1 struct]
    %
    % setflag = 
    %        cutoff: 1
    %      maxclust: 0
    ...
    if ~isempty(criterion)
        ...
        criterion = getParamVal_z(criterion,okcrit,'Criterion');

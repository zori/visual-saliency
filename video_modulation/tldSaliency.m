...
[SBef, S_flicker] = process_saliency(SBef, S_flicker);
...
for i = 2:length(tld.source.idx) % for every frame
    ...
    if i == param.flashL
        [input_img, output_img, meanW, weights, weightsIdx] = modulate_frame(flash, W, weights, weightsIdx);
    ...
    elseif i > param.flashL
        ...
        pyras_modulated = make_pyras(output_img, pyras_modulated);
        [s_modulated, s_flicker_modulated] = get_salimap(pyras_modulated); % TODO(zori) this is flicker BEFORE; would be the same regardless of the modulation type
        [s_modulated, s_flicker_modulated] = process_saliency(s_modulated, s_flicker_modulated);
        write_videos(video_writers, output_img, s_modulated, s_flicker_modulated);
    end

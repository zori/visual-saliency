...
12/11 カーネル利用方式からの利用。強度係数の絶対値の逆数を参照して、最小二乗。出来栄えは微妙

12/14 重み付けの位置を間違えていたことが発覚したので修正
*/
...
cv::Mat MiniMiniKK(const cv::Mat edit,int count,const cv::Mat Kyoudo);
...
char inputfile[] = "TD_3.jpg"; //入力元ファイル名
char maskfile[] = "TD_3_mask.bmp"; //入力マスクファイル名

char Infile[] = "TD_3.jpg"; //最小二乗の理想ファイル名 特に何もなければ入力と同じでよい
...
//			int Inum = depth.data[i];
		...
		//加工後の画像を最小二乗で規格化
		cv::Mat Mini = MiniMiniKK(after,q+1,Kyoudo);
		//加工後の画像出力
		char nameafter[40];
...
cv::Mat MiniMiniKK(const cv::Mat edit,int count,const cv::Mat Kyoudo){
	...
	cv::Mat edit2(rows, cols, CV_8UC3);
		...
		cv::Mat p(rows,cols,CV_64FC1);//重み格納
		double psum = 0.0;

		//
		/*
		for(i=0 ;i<N; i++){
			int pxy = abs(Kyoudo.data[i]-128);
			double pre = 0.0;
			if(pxy < 102){
				pre = 1.0;
				psum++;
		...
		*/
		...
		/*
		for(i=0 ;i<N; i++){
			double pre = 0.0 + abs(Kyoudo.data[i]-128);

			int prey= i/cols;
			int prex = i%cols;		

			p.at<double>(prey, prex) = sqrt(128.0*128.0-pre*pre)/128.0;
		}
		psum = N;
		*/
		...
		for(i=0 ;i<N; i++){
			double pre = abs(Kyoudo.data[i]-128)+0.0/128.0;

			int prey= i/cols;
			int prex = i%cols;		

			p.at<double>(prey, prex) = cbrt(1.0-pre*pre*pre);
		}
		psum = N;
...
*/		
	...
	/*
		//強度係数を段階にわける
		for(i=0 ;i<N; i++){
			double pre = double( abs(Kyoudo.data[i]-128) + 0.0);
			pre = (128.0 -pre)/128.0; //傾斜
			int prey= i/cols;
				int prex = i%cols;		
			p.at<double>(prey, prex) = pre;
			psum += pre;
	...
	*/
	...
	for (i=0; i<N; i++){
		int prey= i/cols;
		int prex = i%cols;		
		double pre = p.at<double>(prey, prex);

	Bsum_xy += double(pre * img2.data[i*3] * edit2.data[i*3]);
    Bsum_x += double(pre * edit2.data[i*3]);
    Bsum_y += double(pre * img2.data[i*3]);
    Bsum_x2 += pre * pow(double(edit2.data[i*3]), 2);

	Gsum_xy += double(pre * img2.data[i*3+1] * edit2.data[i*3+1]);
    Gsum_x += double(pre * edit2.data[i*3+1]);
    Gsum_y += double(pre * img2.data[i*3+1]);
    Gsum_x2 += pre * pow(double(edit2.data[i*3+1]), 2);

	Rsum_xy += double(pre * img2.data[i*3+2] * edit2.data[i*3+2]);
    Rsum_x += double(pre * edit2.data[i*3+2]);
    Rsum_y += double(pre * img2.data[i*3+2]);
    Rsum_x2 += pre * pow(double(edit2.data[i*3+2]), 2);
	}


 
  Ba = (psum * Bsum_xy - Bsum_x * Bsum_y) / (psum * Bsum_x2 - pow(Bsum_x, 2));
  Bb = (Bsum_x2 * Bsum_y - Bsum_xy * Bsum_x) / (psum * Bsum_x2 - pow(Bsum_x, 2));

  Ga = (psum * Gsum_xy - Gsum_x * Gsum_y) / (psum * Gsum_x2 - pow(Gsum_x, 2));
  Gb = (Gsum_x2 * Gsum_y - Gsum_xy * Gsum_x) / (psum * Gsum_x2 - pow(Gsum_x, 2));

  Ra = (psum * Rsum_xy - Rsum_x * Rsum_y) / (psum * Rsum_x2 - pow(Rsum_x, 2));
  Rb = (Rsum_x2 * Rsum_y - Rsum_xy * Rsum_x) / (psum * Rsum_x2 - pow(Rsum_x, 2));
